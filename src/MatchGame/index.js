...
class MatchGame extends Component {
  ...
  thick = () => {
    this.setState(prevstate => ({timers: prevstate.timers - 1}))
  }
  ...
  onClickButtonMach = id => {
    ...
    console.log(id)
    const imgnum = this.onRandomImage()
    const b = imagesList[imgnum].imageUrl
    this.setState({displayImage: b})
    if (id === imagesList[b].id) {
  ...
  onFilterList = () => {
    ...
    console.log(tabName)
    const filterImageList = imagesList.filter(each => each.category === tabName)
    return filterImageList
  }

  onResetButton = () => {
    this.setState({timers: 60, score: 0})
  }

  render() {
    const {tabName, displayImage, score, timers} = this.state
    console.log(tabName)
    console.log(score)
    console.log(timers)
    const displayList = this.onFilterList()
    return (
              ...
              <div>
                ...
                <li className="tim">
                  <img
                    ...
                    alt="timer"
                  />
                  <p className="timer">{timers} sec</p>
                </li>
              </div>
            </nav>
            <div className="matchimagecontainer">
              <img src={displayImage} alt="match" />
            </div>
            <div>
              <p>dddd</p>
            </div>
              ...
              <ul className="gamecard">
                {displayList.map(each => (
                  <MatchCard
                    key={each.id}
                    details={each}
                    onClickButtonMach={this.onClickButtonMach}
                  />
                ))}
              </ul>
            </div>
